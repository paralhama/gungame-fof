...
#define SOUND_TIEDLEAD "music/most_wanted_stinger.wav"
...
{
    name = "[FoF] Gun Game",
    author = "Paralhama, CrimsonTautology, Leonardo",
    description = "Gun Game for Fistful of Frags - Paralhama version",
...
stock ScanConVars()
{
    ...
    //flBonusRoundTime = FloatMax( 0.0, GetConVarFloat( mp_bonusroundtime ) ); // remover comentário da linha 125 e 58
    flBonusRoundTime = 25.0;
}
...
public Event_PlayerDeath( Handle:hEvent, const String:szEventName[], bool:bDontBroadcast )
{
	...
	if( (StrEqual(szWeapon, "fists") ||  StrEqual(szWeapon, "kick-fall") ||  StrEqual(szWeapon, "fists_brass") ||  StrEqual(szWeapon, "physics") ||  StrEqual(szWeapon, "thrown_gun") ||  StrEqual(szWeapon, "flame") )
	...
	else if( StrEqual( szWeapon, "fists_brass" ) )
		strcopy( szWeapon, sizeof( szWeapon ), "weapon_fists" );
	...
	else if( StrEqual( szWeapon, "dynamite_yellow" ) )
		strcopy( szWeapon, sizeof( szWeapon ), "weapon_dynamite_belt" );
	else if( StrEqual( szWeapon, "blast" ) || StrEqual( szWeapon, "prop_dynamic" ) )
...
public Action:Timer_UpdateEquipment( Handle:hTimer, any:iUserID )
{
	...
	else
	{
		...
		new hand = GetEntProp(iClient, Prop_Send, "m_nHandStance");
		if ( hand == 1 && szPlayerWeapon[1][0] != '\0' )
		{
			...
			WritePackString( hPack1, szPlayerWeapon[1] );
			...
			WritePackString( hPack2, szPlayerWeapon[0] );
		}
		else
		{
			//if(g_Timer_GiveWeapon1[iClient] != INVALID_HANDLE) CloseHandle(g_Timer_GiveWeapon1[iClient]);
